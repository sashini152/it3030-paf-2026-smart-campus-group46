...
export default function BookingsPage() {
  ...
  const [listFilter, setListFilter] = useState({ status: '', mineOnly: false })
  ...
  const loadPending = useCallback(async () => {
    try {
      const data = await getJson(`/api/bookings${buildQuery({ status: 'PENDING' })}`)
  ...
  const loadBookings = useCallback(async () => {
    ...
    try {
      const q = buildQuery({
        ...
        userId: listFilter.mineOnly && userId ? userId : undefined,
      })
  ...
  }, [listFilter.mineOnly, listFilter.status, userId, loadPending])
  ...
  async function approve(id) {
    setError(null)
    try {
      await putJson(`/api/bookings/${id}/approve`, {})
      await loadBookings()
    } catch (e) {
      setError(e.message)
    }
  ...
  async function reject(id) {
    ...
    setError(null)
    try {
      await putJson(`/api/bookings/${id}/reject`, { reason: reason.trim() })
      await loadBookings()
    } catch (e) {
      setError(e.message)
    }
              ...
              <tbody>
                {pendingList.map((b) => (
                  <tr key={b.id}>
                    ...
                    <td>{b.requestedByUserId}</td>
                    <td className="hub-table__clip">{b.purpose}</td>
                    <td className="hub-table__actions">
                      <button
                        type="button"
