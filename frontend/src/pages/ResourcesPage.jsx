...
const AVAILABILITY_SEPARATOR = " to ";

function buildAvailabilityWindow(from, to, fallbackText = "") {
  if (from && to) return `${from}${AVAILABILITY_SEPARATOR}${to}`;
  return fallbackText || "";
}

function splitAvailabilityWindow(value) {
  if (!value || !value.includes(AVAILABILITY_SEPARATOR)) {
    return {
      from: "",
      to: "",
    };
  ...
  const [from, to] = value.split(AVAILABILITY_SEPARATOR);

  return {
    from: from?.trim()?.slice(0, 16) || "",
    to: to?.trim()?.slice(0, 16) || "",
  };
}

function formatAvailabilityWindow(value) {
  if (!value) return "-";

  const { from, to } = splitAvailabilityWindow(value);
...
export default function ResourcesPage() {
  ...
  function startEdit(resource) {
    const availability = splitAvailabilityWindow(
  ...
  async function handleSubmit(event) {
    ...
    try {
      if (
        ...
        throw new Error("Please select both Available From and Available To.");
      }

      if (
        form.availabilityFrom &&
        form.availabilityTo &&
        new Date(form.availabilityFrom) >= new Date(form.availabilityTo)
      ) {
        throw new Error("Available To must be after Available From.");
      }

      const availabilityWindows = buildAvailabilityWindow(
        form.availabilityFrom,
        form.availabilityTo,
  ...
  return (
    <div className="hub-page hub-page--wide hub-page--resources space-y-6">
      ...
      {error && (
        <Reveal delay={95}>
          <div
            className="hub-alert hub-alert--error hub-alert--resources"
            role="alert"
          >
            {error}
          </div>
          ...
          <Reveal delay={110}>
            <ParallaxPanel strength={8} className="hub-lift">
              <section className="hub-panel hub-resources-panel">
                ...
                <div
                  className="hub-resource-filter-chips"
                  role="toolbar"
                  aria-label="Quick type filters"
                >
                  <button
                    type="button"
                    onClick={() =>
                ...
                <div className="hub-form-grid hub-form-grid--filters">
                  <label className="hub-field">
                    <span className="inline-flex items-center gap-2">
                      <span>Type</span>
                      <Tooltip
                        text="Use quick chips or dropdown to narrow by resource type."
                        tone="ticket"
                      >
                    ...
                    <label className="hub-field">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setForm((current) => ({
                            ...current,
                            availabilityTo: event.target.value,
                          }))
                        }
                      />
                    </label>
            ...
            <ParallaxPanel strength={12} className="hub-lift">
              <section className="hub-panel hub-resources-panel">
                ...
                {loading ? (
                  ...
                  <p className="hub-muted hub-muted--resources">
                        ...
                        {visibleItems.map((resource, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                {(() => {
                                  const { from, to } = splitAvailabilityWindow(
                                    resource.availabilityWindows,
                                  );
          ...
          <Reveal delay={220}>
            <ParallaxPanel strength={10}>
              ...
              >
                ...
                <div
                  key={activeInsight}
                  className="hub-resource-insight hub-fade-slide"
                >
                    ...
                    {insightSlides.map((slide, index) => (
                      <button
                        ...
                        className={`hub-resource-insight-dot ${
                          index === activeInsight
                            ? "hub-resource-insight-dot--active"
                            : ""
                        }`}
            ...
            <ParallaxPanel strength={8}>
              <section className="hub-panel hub-resources-panel hub-resources-panel--checklist">
                ...
                <ul className="hub-resource-checklist">
                  ...
                  <li>
                    Update OUT_OF_SERVICE entries as soon as faults appear.
                  </li>
